...
func _simulate_single_day() -> void:
	...
	if games_today.is_empty():
		progress_status_label.text = "%s を進行しました / 今日は試合なし" % simulated_date_label
	else:
		progress_status_label.text = "%s を進行しました / 試合数: %d" % [simulated_date_label, games_today.size()]
...
func _simulate_multiple_days(day_count: int) -> void:
	...
	if simulated_days <= 0:
		...
		progress_status_label.text = "%s から %s まで進行 / 試合日: %d / 経過日数: %d / 消化試合数: %d" % [
			start_date_label,
			end_date_label,
			simulated_days,
...
func _refresh_progress_buttons() -> void:
	...
	live_game_button.disabled = auto_progress_active or _get_today_controlled_team_game() == null
...
func _refresh_season_status() -> void:
	...
	match phase:
		...
		_:
			...
			lines.append("年末まで進めると次年度へ移行します")
...
func _refresh_daily_events() -> void:
	...
	if events.is_empty():
		event_detail_label.text = "今日は特別なイベントはありません。"
		return
	var lines: Array[String] = []
...
func _refresh_focus_team_summary() -> void:
	...
	if not focus_team.rotation_ids.is_empty():
		var starter_index: int = maxi(0, (LeagueState.current_day - 1) % focus_team.rotation_ids.size())
...
func _refresh_today_summary() -> void:
	if LeagueState.current_day > LeagueState.get_last_day():
		today_detail_label.text = "今年の日程はすべて消化しています。"
		return

	var games_today: Array = LeagueState.get_games_for_day(LeagueState.current_day)
	if games_today.is_empty():
...
func _refresh_yesterday_summary() -> void:
	...
	if games_yesterday.is_empty():
		yesterday_detail_label.text = "前日の試合結果はありません。"
		return
	...
	for game_value in games_yesterday:
		var game: GameData = game_value
		lines.append("- %s %d - %d %s" % [
			_get_team_name(str(game.away_team_id)),
			int(game.away_score),
			int(game.home_score),
			_get_team_name(str(game.home_team_id))
		])
		if LeagueState.controlled_team_id != "" and (str(game.away_team_id) == LeagueState.controlled_team_id or str(game.home_team_id) == LeagueState.controlled_team_id):
			controlled_game = game
...
func _refresh_note_summary() -> void:
	...
	lines.append("今日の試合へ: 担当球団の当日試合をライブで開く")
