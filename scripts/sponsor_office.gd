...
func _ready() -> void:
	OOTP_SHELL_SCRIPT.install(self, {
		...
		"section_label": "SPONSORS",
		"sub_tab": "sponsors",
		"sub_tabs": [
			{"key": "hub", "label": "FRONT OFFICE", "scene": FRONT_OFFICE_SCENE_PATH},
			{"key": "sponsors", "label": "SPONSORS", "scene": "res://scenes/SponsorOffice.tscn"},
...
func _refresh_view() -> void:
	...
	info_label.text = "%s のスポンサー状況です。" % team.name
	detail_label.text = "現在のスポンサー: %s\nランク: %d\n日次収入: +%d" % [
